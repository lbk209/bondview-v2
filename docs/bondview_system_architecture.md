...
Components are the authoritative analytical representation of bond-market and macroeconomic conditions. Bond Analysis produces Components, ETF Evaluation applies those Components to defined ETF exposures, and Diagnostics explains and validates authoritative results and their dependency lineage.
...
Diagnostics is lineage-scoped by default. It may inspect a Component, an intermediate ETF Evaluation result, or an ETF Evaluation Result together with the authoritative dependency lineage required to explain that target.

Diagnostics may summarize and present authoritative information for human interpretation, but it must not introduce a separate analytical mapping that creates a parallel market assessment, exposure assessment, or decision model.

Comparison of an evaluation with an ETF Exposure Profile remains part of ETF Evaluation rather than a substitute calculation performed by Diagnostics.

Diagnostics is downstream of authoritative calculations and must not become an upstream dependency of Bond Analysis or ETF Evaluation. Detailed diagnostic behavior is defined in Section 5.

---

# 3. Component Architecture

Components are the principal analytical boundary between Bond Analysis and downstream consumers.
...
The intended backward traceability direction is:
...
Macro Adjustment should normally consume the Core Evaluation Result and relevant macroeconomic Components rather than independently reinterpret the entire ETF Exposure Profile. If a specific macro rule requires exposure information not sufficiently represented or referenced by the Core Evaluation Result, the evaluator-specific Result Contract or design should make that requirement explicit.
...
Positioning Overlay does not imply that the underlying economic thesis is incorrect. It modifies implementation willingness or intensity around the Exposure Evaluation Result without redefining that underlying assessment.
...
Diagnostics inspects an authoritative result together with the actual dependency lineage needed to explain it.

A diagnostic target may be:

- a Component;
- an intermediate Core Evaluation Result;
- a Constituent Evaluation Result;
- an Exposure Evaluation Result;
- a positioning-adjusted result;
- an ETF Evaluation Result.

For a diagnostic target, Diagnostics may inspect:

- the target result;
- intermediate evaluation results in its calculation path;
- relevant ETF Exposure Profile characteristics referenced by that path;
- relevant Component Values and States;
- supporting Features and Raw Observations;
- historical frequency and persistence;
- State transitions;
- historical episodes;
- Component-to-Evaluation and evaluation-stage traceability;
- sensitivity to model parameters and horizons.

The default diagnostic boundary is the target's dependency lineage. This limits which analytical variables and dependencies Diagnostics may introduce; it does not limit Diagnostics to the target date. Historical realizations of those same lineage entities may be inspected for context and sanity checking.

Diagnostics should not introduce unrelated supplemental market or macro data merely because those data are available.

Diagnostics may summarize authoritative Components and evaluation results in human-readable form. Such presentation remains diagnostic and must not create a separate Rule Mapping, market assessment, or exposure assessment that functions as a second authoritative model.

For a selected date or period, Diagnostics should expose enough lineage and supporting information for an expert to determine whether the resulting Component or evaluation is economically plausible.
...
Raw Observations, Features, Components, intermediate ETF Evaluation results, ETF Evaluation Results, and Diagnostic Results should remain conceptually distinguishable even when Diagnostics traverses their lineage for explanation.
...
Model-specific economic meaning belongs in the Component, Evaluation, or other responsibility that uses the mechanic.

Equivalent mechanics may be reused without collapsing different analytical concepts or semantic stages into one architectural concept.
...
Intermediate evaluation results that carry model-significant semantics, including Core Evaluation Results and Constituent Evaluation Results, should remain explicit and traceable even when they are not top-level Module Result Boundaries.
